...
# Function to get the location details of an IP address using ip-api.com API
def get_ip_location(ip_address):
    url = f"http://ip-api.com/json/{ip_address}"
    response = requests.get(url)
    if response.status_code == 200:
        data = response.json()
        return data.get('query', 'N/A'), data.get('city', 'N/A'), data.get('regionName', 'N/A'), data.get('country', 'N/A')
    else:
